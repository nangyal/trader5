"""
Predict patterns on October 2025 data using August-trained model
Uses DOGEUSDT-trades-2025-10.csv and the trained model
"""
...
def main_prediction():
    """Main function to predict patterns on October data"""
    print("="*70)
    print("PATTERN PREDICTION ON OCTOBER 2025 DATA")
    print("Using August-trained model")
    print("="*70)
    
    # Configuration
    october_csv = 'data/DOGEUSDT-trades-2025-10.csv'
    ...
    # 1. Resample October tick data to hourly
    df_october = resample_ticks_to_hourly(october_csv)
    ...
    # 3. Make predictions on October data
    print("\n--- Making Predictions on October Data ---")
    predictions, probabilities = classifier.predict(df_october)
    
    # 4. Calculate pattern strength scores
    print("\n--- Calculating Pattern Strength Scores ---")
    strength_scores = []
    for i in range(len(df_october)):
        if predictions[i] != 'no_pattern':
            score = PatternStrengthScorer.calculate_pattern_strength(
                df_october, predictions[i], i
            )
        else:
            score = 0.0
    ...
    print("\n" + "="*70)
    print("PATTERN DETECTION RESULTS - OCTOBER 2025")
    print("="*70)
    ...
    print("\n--- Running Backtest on October Data ---")
    ...
    backtest_results = backtester.run_backtest(
        df_october, predictions, probabilities, strength_scores
    )
    ...
    if backtest_results:
        ...
        output_dir = f"october_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        os.makedirs(output_dir, exist_ok=True)
        
        # Plot equity curve
        equity_curve_path = os.path.join(output_dir, 'equity_curve_october.png')
        ...
        display_size = min(500, len(df_october))
        dashboard.create_candlestick_chart(
            df_october.head(display_size), 
        ...
        print(f"  - equity_curve_october.png")
    ...
    print("\n" + "="*70)
    print("SUMMARY - OCTOBER 2025 BACKTEST")
    print("="*70)
    print(f"Training Period: August 2025 (hourly data)")
    print(f"Testing Period: October 2025 (resampled to hourly)")
    print(f"Total Candles Analyzed: {len(df_october)}")
    ...
    return df_october, predictions, backtest_results
