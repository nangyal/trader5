...
class EnhancedForexPatternClassifier:
        ...
        try:
            ...
            self.model = xgb.XGBClassifier(
                **best_params,
                ...
                device='cuda:0',     # FIXED: XGBoost 2.0+ format (cuda:0 or gpu)
                predictor='gpu_predictor',  # GPU for prediction too
                scale_pos_weight=weight_dict[1] if len(weight_dict) > 1 else 1
            )
    ...
    def _optimize_hyperparameters(self, X, y):
        ...
        model = xgb.XGBClassifier(
            random_state=42,
            tree_method='hist',
            device='cpu'  # FIXED: CPU for hyperparameter search (more stable)
        )
