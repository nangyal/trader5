...
class HedgingBacktestEngine:
    ...
    def __init__(self, 
                 ...
                 enable_hedging=True,
                 hedge_threshold=0.15,          # Activate hedge when drawdown > 15%
                 hedge_recovery_threshold=0.05,  # Close hedge when drawdown recovers below 5%
                 hedge_ratio=0.5,               # Hedge 50% of net long exposure
                 max_correlation_hedge=0.7,      # Max correlation for counter-hedge
                 dynamic_hedge=True,             # Adapt hedge threshold using volatility (ENABLED by default)
                 volatility_window=20,           # Bars used for volatility estimate
                 min_hedge_threshold=0.10,       # Lower bound when volatility high
                 max_hedge_threshold=0.25,       # Upper bound when volatility low
                 drawdown_basis='capital',       # 'capital' or 'equity' (mark-to-market)
                 use_tiered_risk=True,           # NEW: Tiered compounding (safer)
                 use_fixed_risk=False,           # NEW: Use fixed initial capital for risk (no compounding)
                 max_position_pct=0.20):         # NEW: Max position size as % of capital (default 20%)
        
        self.initial_capital = initial_capital
        self.risk_per_trade = risk_per_trade
        self.take_profit_multiplier = take_profit_multiplier
        self.use_tiered_risk = use_tiered_risk
        self.use_fixed_risk = use_fixed_risk  # NEW
        self.max_position_pct = max_position_pct  # NEW: Max 10% or 20% per position
    ...
    def get_tiered_risk_percentage(self, capital):
        """
        Calculate risk percentage based on capital tiers (safer compounding)
        $10k-20k: 2.0% risk
        $20k-30k: 1.5% risk  
        $30k-50k: 1.0% risk
        $50k+:    0.75% risk
        ...
        if capital < initial * 2:  # < $20k
            return 0.02
        elif capital < initial * 3:  # $20k-30k
            return 0.015
        elif capital < initial * 5:  # $30k-50k
            return 0.01
        else:  # $50k+
            return 0.0075
    ...
    def calculate_pattern_targets(self, pattern_type, entry_price, high, low, recent_data=None):
        ...
        targets = {
            'ascending': {'sl_pct': 0.008, 'tp_pct': 0.012},  # Tighter: -0.8% SL, +1.2% TP
            'descending': {'sl_pct': 0.008, 'tp_pct': 0.012},
            'symmetrical': {'sl_pct': 0.010, 'tp_pct': 0.015},
            'double': {'sl_pct': 0.010, 'tp_pct': 0.015},
            'head': {'sl_pct': 0.012, 'tp_pct': 0.018},
            'cup': {'sl_pct': 0.010, 'tp_pct': 0.015},
            'wedge': {'sl_pct': 0.009, 'tp_pct': 0.013},
            'flag': {'sl_pct': 0.008, 'tp_pct': 0.012},
        }
        ...
        # Trend alignment with EMA filter
        if recent_data is not None and len(recent_data) >= 50:
            ...
            # LONG-ONLY: Skip if price below EMA(50)
            if current_price < ema_50:
                return 0, 0, 'skip', None
            
            # Calculate ATR for volatility filter
            high = recent_data['high'].values[-14:]
            low = recent_data['low'].values[-14:]
            close = recent_data['close'].values[-14:]
            ...
            # Skip if volatility too low (<0.5% ATR)
            if atr_pct < 0.5:
                return 0, 0, 'skip', None
            ...
            # OPTIMIZED v3: Remove Descending Triangle (33.3% accuracy - bearish pattern)
            # Only trade BULLISH patterns with good accuracy
            bullish_patterns = ['ascending', 'symmetrical']  # CUP & DESCENDING REMOVED
            bearish_patterns = ['descending', 'wedge']  # Skip these in LONG-ONLY
            
            is_bullish = any(bp in base_pattern for bp in bullish_patterns)
            is_bearish = any(bp in base_pattern for bp in bearish_patterns)
            
            # Skip bearish patterns completely (Descending Triangle etc.)
            if is_bearish:
                return 0, 0, 'skip', None
            
            # LONG-ONLY: Only bullish patterns in uptrend
            if trend == 'up' and is_bullish:
                direction = 'long'
            else:
                # Skip: trend misalignment
                return 0, 0, 'skip', None
        else:
            # Default: LONG-ONLY for bullish patterns only
            bullish_patterns = ['ascending', 'symmetrical']
            bearish_patterns = ['descending', 'wedge']
            
            # Skip bearish patterns
            if any(bp in base_pattern for bp in bearish_patterns):
                return 0, 0, 'skip', None
    ...
    def _compute_dynamic_threshold(self):
        """Compute a volatility-adjusted hedge activation threshold.
        Uses rolling equity returns standard deviation to scale threshold
        within [min_hedge_threshold, max_hedge_threshold]. High volatility → lower threshold.
        """
        if len(self.equity_curve) < self.volatility_window + 2:
            return self.hedge_threshold  # Not enough data – keep static
        equity = np.array(self.equity_curve[-(self.volatility_window+1):])
        returns = np.diff(equity) / equity[:-1]
        vol = np.std(returns)
        # Normalize volatility relative to a baseline (0.01 typical hourly drift)
        # Clamp between 0 and 1
        norm_vol = min(1.0, max(0.0, vol / 0.03))  # 3% std = high volatility
        # Invert: higher vol -> lower threshold
    ...
    def should_hedge(self, capital, peak_capital, equity=None, peak_equity=None):
        ...
        Optionally adapts hedge_threshold to volatility if dynamic_hedge=True.
        """
        if not self.enable_hedging or peak_capital <= 0:
            return False
        if self.dynamic_hedge:
            adaptive_threshold = self._compute_dynamic_threshold()
        else:
            adaptive_threshold = self.hedge_threshold
    ...
    def should_close_hedge(self, capital, peak_capital, equity=None, peak_equity=None):
        ...
        Uses configurable hedge_recovery_threshold.
        """
        if peak_capital <= 0:
            return False
        if self.drawdown_basis == 'equity' and equity is not None and peak_equity:
    ...
    def create_hedge_trade(self, active_trades, current_price, index):
        ...
        if current_price <= 0:
            return None  # Safety: invalid price
    ...
    def run_backtest(self, df, predictions, probabilities, pattern_strength_scores=None):
        ...
        print(f"Hedge Threshold (base): {self.hedge_threshold*100:.0f}% drawdown")
        if self.dynamic_hedge:
            print(f"Dynamic Hedging: ENABLED (volatility adaptive)")
        else:
            print(f"Dynamic Hedging: disabled")
        print(f"Hedge Ratio: {self.hedge_ratio*100:.0f}% of exposure")
        print(f"Hedge Recovery Threshold: {self.hedge_recovery_threshold*100:.0f}% drawdown")
        ...
        for i in range(len(df)):
            ...
            # Compute mark-to-market equity if needed
            current_price = current_row['close']
            unrealized_main = 0.0
            for t in active_trades:
                if t.get('is_hedge', False):
                    continue
                # LONG-only main trades
            ...
            # Auto-close hedges if no active main trades remain
            ...
            if pattern_strength_scores is not None and pattern_strength_scores[i] < 0.7:
                self.equity_curve.append(capital)
                continue
            ...
            # Use TIERED RISK instead of fixed 2%
            # If use_fixed_risk is True, calculate risk based on INITIAL capital (no compounding)
            ...
            # LONG-ONLY (direction currently always 'long' unless future extension)
            risk_per_unit = entry_price - stop_loss
            
            position_size = risk_amount / risk_per_unit if risk_per_unit > 0 else 0
            
            # CRITICAL FIX: Limit position size to prevent excessive leverage
            # If use_fixed_risk, limit based on INITIAL capital (no compounding leverage)
            # Otherwise limit based on current capital
            # Use configurable max_position_pct (default 20%, can be set to 10% for shared capital)
            if self.use_fixed_risk:
                max_position_value = self.initial_capital * self.max_position_pct  # Fixed: e.g., 10% of $200 = $20
            else:
                max_position_value = capital * self.max_position_pct  # Growing: 10% or 20% of current capital
            
            position_value = position_size * entry_price
            
            if position_value > max_position_value:
                # Scale down position to fit capital constraint
                ...
                if trade['status'] == 'open' and not trade.get('is_hedge', False):
                    if trade['direction'] == 'long':
                        if current_row['low'] <= trade['stop_loss']:
                            # Calculate precise P&L: (exit - entry) * position_size
                        ...
                        if current_row['high'] >= trade['stop_loss']:
                            # SHORT: P&L = (entry - exit) * position_size
                    ...
                    trade['pnl'] = pnl
                    # FIXED: Calculate percentage based on entry value, not risk amount
    ...
    def analyze_results(self):
        ...
        # Pattern-specific analysis (excluding hedges) - robust to missing column
        ...
        # Vectorized win rate calculation (no groupby.apply)
